...
# Login route
from models import User, LoginActivity  # Assuming you have these models defined.

@login_bp.route('/api/login', methods=['POST'])
def login():
    user_data = request.json
    user_name = user_data.get('user_name')  # Assuming login uses email
    password = user_data.get('password')

    if not user_name or not password:
        return jsonify({"message": "Both User Name and password are required."}), 400

    # Query the User collection to find the user by email
    user = User.objects(user_name=user_name).first()

    if not user:
        return jsonify({"message": "User Name not found."}), 400
    ...
    if not verified_email or not verified_mobile:
        return jsonify({"message": "Please verify your email and mobile before logging in."}), 403
    ...
    login_activity = LoginActivity(
        user_name=user_name,  # Assuming the model has a reference to the user
    ...
    # Successful login
    return jsonify({"message": f"Login successful. Welcome, {full_name}!"}), 200